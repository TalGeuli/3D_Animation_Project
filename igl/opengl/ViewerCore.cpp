  ...
  bool update_matrices)
{
  ...
  if(update_matrices)
  {
    ...
    if (orthographic)
    {
      ...
      float h = tan(camera_view_angle/360.0 * igl::PI) * (length); //camera angle
    ...
    else
    {
      ...
      frustum(-fW, fW, -fH, fH, camera_dnear, camera_dfar,proj); //set where near and far
    }
  }
...
IGL_INLINE igl::opengl::ViewerCore::ViewerCore()
{
  ...
  camera_eye << 0, 0, 18; ///CHANGED - original value (0,0,5)
  camera_center << 0, 0, 0;
  camera_up << 0, 1, 0;
